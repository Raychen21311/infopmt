...
7) 若 id = 'A2.1、A2.2、A2.3 或 A2.4'，原則上預審表結果為不適用即為不適用，並提醒使用者再次核實。
8) 若 id = 'A2.1、A2.2、A2.3 或 A2.4'，原則上預審表結果為不適用即為不適用，並提醒使用者再次核實。
9) 若 id = 'D5、D6'，符合兩條件中之一件即可。
10) 若 id = 'D15'，註5.作業需求必須納入之制式文句：
(1)本專案所使用之系統軟體(如作業系統、資料庫軟體等)或瀏覽器軟體版本更新時，於本部通
知限期內，廠商須對應用系統進行修正。
(2)支援系統備援及緊急回復工作，並視本部要求配合執行系統遷移不同伺服主機作業。
(3)維持本專案系統與其他相關系統整合介面之正常運作（含介接作業所需之欄位增修）。
(4)本專案系統建置過程須適當導入虛擬化技術並預留日後擴充之可能性，以減少各系統重複建
置資源之成本。
(5)提供本專案系統使用者帳號清查紀錄表。
(6) 提供本專案系統使用者帳號登出、入、異常紀錄查詢列表。
(7)本專案系統之資料異動，需記錄異動內容、操作者、時間、IP 等資訊，以供後續追蹤查詢。
(內含重要資料之系統時請列入)
(8)本專案若屬新建置系統於資料庫規格設計時須引用政府資料標準平臺
...
(1)GIS（地理圖資系統）：
A.空間資訊須採用全國通用之坐標系統（TWD97、WGS84 EPSG:3857、WGS84 EPSG:4326）。
B. 輸入資料格式須至少為CSV、XLS、XLSX、XML、JSON、GeoJSON、KML、KMZ、或SHP，輸出
資料格式至少為CSV、GeoJSON、KML、KMZ、或SHP。輸入及輸出之資料內容並須符合國發
會「政府資料品質提升機制運作指引」之規範。
C. 輸出資料須有固定下載連結，或提供符合「國發會共通性應用程式介面規範」之介接方式。
D.建置地圖服務如有使用各式主題圖資、定位查詢、及分析功能之需求，應優先使用內政部
TGOS 提供之各式服務資源（https://www.tgos.tw）；另使用之底圖，應使用內政部國土測
繪中心提供之臺灣通用電子地圖各式底圖資源（https://maps.nlsc.gov.tw/）。
E.匯入系統之資料內容如包括地址，則須自動轉為坐標。
F.使用付費網路資源建置系統或圖台，購置授權數量須符合預估用量。
G.使用付費軟體建置GIS 伺服器，購買授權數量須符合預估計算與發佈能量需求。
(2)OPENDATA（政府資料開放）：
A.資料(庫)欄位設計須引用政府資料標準平臺(https://schema.gov.tw/)所公布相關領域
資料標準之資料欄位命名及格式定義。
B. 輸入資料格式需至少為CSV、XLS、XLSX、XML 或JSON，輸出資料格式至少為CSV、XML 或
JSON。輸入及輸出之資料內容並須符合國發會「政府資料品質提升機制運作指引」之規範。
(3)MYDATA（數位服務個人化）：
A.應依據國家發展委員會「數位服務個人化平臺介接作業試辦要點」辦理，提供民眾下載及
運用其個人化資料，並保障資訊安全及個人隱私權益。
B.具資料提供者角色之系統，須於數位服務個人化(MyData)管理後臺登入填寫資料傳輸項目
及開發介接程式，經當事人完成身分驗證及同意後，傳輸提供該個人化資料。
C.具資料提供者角色之系統，應參照國家發展委員會範例製作及上傳「數位服務個人化
(MyData)介接資料檔案規格書」。
D.具服務提供者角色之系統，須於數位服務個人化(MyData)管理後臺登入填寫服務項目，及
參照已開放之數位服務個人化(MyData)介接資料檔案規格書開發介接程式，經當事人完成
身分驗證及同意後，取得該當事人個人化資料並處理運用。
E.具服務提供者角色之系統，應於線上服務揭示服務條款，包含服務名稱、服務目的與內容、
資料集項目、資料儲存與使用方式、當事人相關權利等內容。
F.應產製當事人資料傳輸相關紀錄並保存至少1 年，紀錄內容至少包含傳輸時間、傳輸對象、
當事人身分、資料傳輸成功與否等。
G.開發數位服務個人化(MyData)平臺介接程式，請參考「數位服務個人化(MyData)應用規範」
