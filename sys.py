...
【系統標準 ID 規則（僅供你計算 std_id）】
- 章節→代號：案件性質→A；現況說明→B；資安需求→C；作業需求→D；產品交付→E；其他重點→F
- 主號→「A1/B2/C3/D4/E5/F6…」；有次號→例如「A2.1/B1.2/C3.2…」
- 若檢出「案件性質」類型勾選（開發建置/系統維運/功能增修/套裝軟體/硬體/其他），請額外新增一列 A0：
  {{
    "id": "A0", "item": "案件性質（六選一）",
    "status": "（填被勾選的類型字樣）",   # ★ A0 例外：非「符合/不適用」，而是六選一字面
