...
一：「本案採購金額多少萬元，包含系統維運、功能增修等。」(更新金額多少萬元即可)
二：「資訊系統之維運費用應逐年遞減，廠商報價如有增長，可請廠商於本案之期末報告提供系統使用效益指標，做為次年維運費用成長之判斷。」(請勿修改，直接回復)
...
def make_reply_prompt(corpus_text: str) -> str:
    reply_ad = """
請依照使用者上傳文字生成建議回復，回復須包含：
一：本案採購金額多少萬元，包含系統維運、功能增修等。
二：資訊系統之維運費用應逐年遞減，廠商報價如有增長，可請廠商於本案之期末報告提供系統使用效益指標，做為次年維運費用成長之判斷。
額外參考選項：
「有關醫院資料治理工作內容，請參閱本部醫療資訊大平台之醫療資訊標準，如FHIR、LOINC、SNOMED CT、RxNorm，且符合三大AI中心、SMART on FHIR等作業事項。另如有TWCDI及IG需求，可至該平台提案。」
「有關檢核表，已請單位承辦人酌修完畢制式文句檢核內容。」
""".strip()
